...
public class UddfDeserialiserTests
{
	private readonly UddfDeserialiser _uddfDeserialiser;
	
	public UddfDeserialiserTests()
	{
		_uddfDeserialiser = new(new());
	}
	
		[Fact]
		public void LinksAreResolvedWhenDeserialising()
		{
			const string xml = """
	                           <uddf version="3.2.0">
	                               <mediadata>
		...
		public void CanDeserialiseFromStream()
		{
			...
			UniversalDiveDataFormatRoot uddf = _uddfDeserialiser.Deserialise<UniversalDiveDataFormatRoot>(stream);
			uddf.ShouldNotBeNull();
			uddf.MediaData.ShouldNotBeNull();
		}
		
		[Fact]
		public void CanDeserialiseNonRootObject()
		{
			const string xml = """
								<image id="img_flatfeet">
								   <title>Arno's flatfeet...</title>
